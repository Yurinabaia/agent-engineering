---
name: implement-fix
description: Implementa a correção de um bug (issue do Jira ou do GitHub) a partir do documento de RCA, adiciona teste de regressão e valida com dotnet build/test. Use depois que a RCA existir.
argument-hint: <CHAVE-JIRA | github-issue-id>
allowed-tools: Read, Write, Edit, Bash(dotnet:*), Bash(git:*)
---
...
All changes complete and validated. Ready for the `commit` skill.
...
- If the RCA document is missing or incomplete, request it be created first with the `rca` skill for issue #$ARGUMENTS
