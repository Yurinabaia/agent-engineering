...
4. **Start server in background**
   ...
   ```bash
   dotnet run --project <Sln>.Api --urls http://localhost:5124 &
   SERVER_PID=$!
   ```
...
1. **Spawn Agent 1 using Task tool**
   ...
   3. Restaurar dependências: dotnet restore <Sln>.Api/<Sln>.Api.sln
   4. Subir a API: dotnet run --project <Sln>.Api --urls http://localhost:5124 &
...
2. **Spawn Agent 2 using Task tool (simultaneously with Agent 1)**
   ...
   3. Restaurar dependências: dotnet restore <Sln>.Api/<Sln>.Api.sln
   4. Subir a API: dotnet run --project <Sln>.Api --urls http://localhost:5125 &
...
- Exact ports and the entry project may differ — check `launchSettings.json`, the AppHost and the
