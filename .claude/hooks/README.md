...
|---|---|---|---|
| `pre_tool_use.py` | **PreToolUse** | Blocks reading/writing/searching a real env file (committed `.env.example` templates are allowed), blocks `rm -rf`, and blocks hand-writing a **new** EF Core migration file (generated artifacts must come from `dotnet ef migrations add`; editing an existing migration is still allowed). Prints the reason to stderr and `exit(2)` → the tool is stopped and the agent is told why, so it adapts. | **Yes** — this is the guarantee |
