...
Three guarantees ship here:
  ...
  3. The agent can never hand-write a NEW EF Core migration file. Migrations are
     generated by `dotnet ef migrations add` or they lie about the model snapshot.
     Editing an EXISTING migration is still allowed (custom SQL is legitimate).
...
# EF Core migrations are generated artifacts: the .cs file, its .Designer.cs and the
# model snapshot must agree. A hand-written migration compiles and then silently
# diverges from the snapshot, and the next generated migration inherits the lie.
MIGRATION_PATH = re.compile(r"/Migrations/.*\.cs$", re.IGNORECASE)
BLOCKED_MIGRATION_MESSAGE = (
    "BLOCKED: migrations are generated, not hand-written.\n"
    "Run: dotnet ef migrations add <Nome> -p <Sln>.Infrastructure -s <Sln>.Api"
)
...
def is_handwritten_migration(tool_name: str, tool_input: dict) -> bool:
    """True if the call would CREATE a new EF Core migration file by hand."""
    if tool_name != "Write":
        return False
    path = tool_input.get("file_path", "").replace("\\", "/")
...
def main() -> None:
    try:
        ...
        if is_handwritten_migration(tool_name, tool_input):
            print(BLOCKED_MIGRATION_MESSAGE, file=sys.stderr)
            sys.exit(2)
